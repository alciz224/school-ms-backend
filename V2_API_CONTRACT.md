...
Backend returns the following user object (via `UserSerializer`):

- `id`: user UUID/string
- `email`: nullable email address
- `phone`: nullable phone number
- `first_name`, `last_name`, `full_name`: user's name fields
- `is_verified`: boolean indicating if at least one contact method is verified
- `is_active`: boolean indicating if the account is active
- `security`: object containing:
  - `score`: integer (0-100) representing account security score
  - `level`: string ("low", "medium", or "high")
- `available_roles`: array of role strings the user can select (e.g., `["student", "teacher"]`)
- `active_role`: nullable string representing the currently selected role from the session

### Backend Implementation

The backend uses `UserSerializer` from `domain.account.api.serializers.user` which:
- Returns consistent user object across all V2 auth endpoints
- Includes `active_role` from session via `request.session[ACTIVE_ROLE_SESSION_KEY]`
- Calculates `available_roles` using `UserRoleSelector.get_available_roles(user)`
- Computes `security` score/level from user model properties

The `select-role` endpoint:
- Validates requested role is in `available_roles`
- Stores selected role in session: `request.session[ACTIVE_ROLE_SESSION_KEY] = role`
- Subsequent requests return updated `active_role` in user object
...
**Note:** This endpoint returns the same user object structure as `register` and `login` using `UserSerializer`. For more detailed user information (masked fields, verification details, backup phone, etc.), use the V1 `/api/auth/me/` endpoint which returns `UserDetailSerializer`.
...
- Backup phone added: +15 points
- Security questions: +10 points each (max 3)
